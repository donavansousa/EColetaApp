...
const CreatePoint = () =>{
    ...
    const[ufs,setUfsInitials]=useState<string[]>([]);
    const[selectedUf,setSelectedUf]=useState('0');

    const[citys,setCitysInitials]=useState<string[]>([]);
   ...
   async function handleSubmit(event: FormEvent){
        ...
        const {name,email,whatsapp}=formInput;
        const uf=selectedUf;
            ...
            <form onSubmit={handleSubmit}>
                ...
                <fieldset>
                    ...
                    <ul className="items-grid">
                        {items.map(item=>(
                            <li key={item.id} onClick={()=> handleItems(item.id)} 
